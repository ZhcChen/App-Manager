...
describe("electron-builder branding config", () => {
  it("does not define a top-level icon that can override mac.icon", () => {
    expect(configText).not.toMatch(/^icon:/m);
  });

  it("bundles runtime Dock PNG resources under the expected brand directory", () => {
    expect(configText).toContain(`to: ${BRAND_ICON_RESOURCE_DIR}`);
    expect(configText).toContain(`- ${RELEASE_DOCK_ICON_NAME}`);
    expect(configText).toContain(`- ${DEV_DOCK_ICON_NAME}`);
